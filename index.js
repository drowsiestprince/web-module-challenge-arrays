...
function copy(originalFlavors){
  let newArray = [];
  newArray = originalFlavors.slice();
  return newArray;
}
...
function is31Flavors(originalFlavors){
  const includes31 = originalFlavors.length;
  if(includes31 === 31)
  {
    return true;
  }
...
function removeFlavorByName(origFlavors, removeFlavor){
  let filteredFlavors = [];
  for(let i = 0; i < origFlavors.length; i++) 
  {
    if(origFlavors[i] !== removeFlavor)
    {
      filteredFlavors.push(origFlavors[i]);
    }
  }
...
function getAverageWordLength(originalFlavors){
  ...
  for(let i = 0; i < originalFlavors.length; i++) 
  {
     countArray.push(originalFlavors[i].length); 
  }
  const average = countArray => countArray.reduce((a, b) => a + b) / array.length;
  return average;
}
